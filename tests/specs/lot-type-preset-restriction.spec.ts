...
test('a preset restricted to Lot Type A never grades a lot as Type B or C', async ({ page, context }, testInfo) => {
  ...
  // Lake Louisa: the fastest file to grade — see [[feedback_new_test_workflow]].
  ...
  // Groups get the Type-A-only preset; Zones/Ponds get the default constraint (dynamic — depends
  // on the file and how grading splits it into Areas, so this walks whatever's actually there).
  ...
  for (const item of groupItems) {
    ...
    // Extra (non-authoritative) context for the failure message — see the top-of-file comment on
    // why this alone isn't trusted as the actual check.
    ...
    const screenshotPath = testInfo.outputPath(`lot-type-check-${item.label}.png`);
    ...
    // Back to the tree before the next entry — a Group's solution view has no "View all" of its
    // own (that's on the tree panel this replaced), so leaving needs "Back" first.
  ...
  expect(
    violations,
    `Expected every lot graded with "${PRESET_NAME}" (Lot Type A only) to render as Type A on the ` +
      `map, but found a "B" or "C" in the recognized label text for: ${JSON.stringify(violations, null, 2)}\n` +
